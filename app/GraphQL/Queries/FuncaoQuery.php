...
class FuncaoQuery extends Query
{
    ...
    {
        return Auth::check() && Auth::user()->can('funcao:view');
    }

    public function type(): Type
    {
        return GraphQL::paginate('Funcao');
    }

    public function args(): array
    {
        return [
            'filter' => ['name' => 'filter', 'type' => GraphQL::type('FuncaoFilter')],
            'order' => ['name' => 'order', 'type' => GraphQL::type('FuncaoOrder')],
    ...
    public function resolve($root, $args, $context, ResolveInfo $resolveInfo, Closure $getSelectFields)
    {
        ...
        $query = Filter::apply(
            $args['filter'] ?? [],
            Funcao::with($fields->getRelations())->select($fields->getSelect())
        );
