...
CREATE TABLE IF NOT EXISTS `Cartoes` (
  ...
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `UK_FormaPagto_Nome` (`FormaPagtoID` ASC, `Bandeira` ASC),
...
CREATE TABLE IF NOT EXISTS `Zonas` (
  ...
  UNIQUE INDEX `UK_Bairro_Nome` (`BairroID` ASC, `Nome` ASC),
...
CREATE TABLE IF NOT EXISTS `Contas` (
  ...
  `Estado` ENUM('Analise', 'Ativa', 'Paga', 'Cancelada', 'Desativada') NOT NULL DEFAULT 'Ativa' COMMENT 'Informa o estado da conta, desativa quando agrupa[N:Estado][G:o]',
...
CREATE TABLE IF NOT EXISTS `Pagamentos` (
  ...
  `Estado` ENUM('Aberto', 'Aguardando', 'Analise', 'Pago', 'Disputa', 'Devolvido', 'Cancelado') NOT NULL DEFAULT 'Aberto' COMMENT 'Informa qual o andamento do processo de pagamento[N:Estado][G:o][F:self::ESTADO_ABERTO]',
...
CREATE TABLE IF NOT EXISTS `Cupons` (
  ...
  `TipoDesconto` ENUM('Valor', 'Porcentagem') NOT NULL COMMENT 'Informa se o desconto será por valor ou porcentagem[G:o][N:Tipo de desconto]',
  ...
  `FuncaoPedidos` ENUM('Menor', 'Igual', 'Maior') NOT NULL DEFAULT 'Maior' COMMENT 'Informa a regra para decidir se a quantidade de pedidos permite usar esse cupom[G:a][N:Função de limite por pedidos]',
  ...
  `FuncaoValor` ENUM('Menor', 'Igual', 'Maior') NOT NULL DEFAULT 'Maior' COMMENT 'Informa a regra para decidir se o valor do pedido permite usar esse cupom[G:a][N:Função de limite por valor]',
...
CREATE TABLE IF NOT EXISTS `Metricas` (
  ...
  `Tipo` ENUM('Entrega', 'Atendimento', 'Producao', 'Apresentacao') NOT NULL COMMENT 'Tipo de métrica que pode ser velocidade de entrega, quantidade no atendimento, sabor da comida e apresentação do prato[G:o][N:Tipo de métrica]',
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Viagens_AFTER_INSERT` AFTER INSERT ON `Viagens` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Viagens_AFTER_UPDATE` AFTER UPDATE ON `Viagens` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Viagens_AFTER_DELETE` AFTER DELETE ON `Viagens` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Cheques_AFTER_INSERT` AFTER INSERT ON `Cheques` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Cheques_AFTER_UPDATE` AFTER UPDATE ON `Cheques` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Cheques_AFTER_DELETE` AFTER DELETE ON `Cheques` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Cupons_AFTER_INSERT` AFTER INSERT ON `Cupons` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Cupons_AFTER_UPDATE` AFTER UPDATE ON `Cupons` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Cupons_AFTER_DELETE` AFTER DELETE ON `Cupons` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Metricas_AFTER_INSERT` AFTER INSERT ON `Metricas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Metricas_AFTER_UPDATE` AFTER UPDATE ON `Metricas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Metricas_AFTER_DELETE` AFTER DELETE ON `Metricas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Avaliacoes_AFTER_INSERT` AFTER INSERT ON `Avaliacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Avaliacoes_AFTER_UPDATE` AFTER UPDATE ON `Avaliacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Avaliacoes_AFTER_DELETE` AFTER DELETE ON `Avaliacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
