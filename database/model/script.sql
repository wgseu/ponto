...
CREATE TABLE IF NOT EXISTS `Carteiras` (
  ...
  `Tipo` ENUM('Bancaria', 'Financeira', 'Credito') NOT NULL COMMENT 'Tipo de carteira, \'Bancaria\' para conta bancária e \'Financeira\' para carteira financeira da empresa ou de sites de pagamentos[N:Tipo][G:o][S:S][E:Bancária|Financeira|Crédito]',
...
CREATE TABLE IF NOT EXISTS `Formas_Pagto` (
  ...
  `Tipo` ENUM('Dinheiro', 'Credito', 'Debito', 'Cheque', 'Conta', 'Saldo') NOT NULL COMMENT 'Tipo de pagamento[N:Tipo][G:o][E:Dinheiro|Cartão|Cheque|Conta|Crédito|Transferência][S:S]',
...
CREATE TABLE IF NOT EXISTS `Funcoes` (
  ...
  UNIQUE INDEX `Descricao_UNIQUE` (`Descricao` ASC))
ENGINE = InnoDB
COMMENT = 'Função ou cargo de um funcionário[N:Função|Funções][G:a][L:AlterarConfiguracoes][K:MZ\\Employee|MZ\\Employee\\][H:SyncModel]';
...
CREATE TABLE IF NOT EXISTS `Funcionarios` (
  ...
  CONSTRAINT `FK_Funcionarios_Clientes_ClienteID`
    ...
    ON UPDATE RESTRICT)
ENGINE = InnoDB
COMMENT = 'Funcionário que trabalha na empresa e possui uma determinada função[N:Funcionário|Funcionários][G:o][L:CadastroFuncionarios][K:MZ\\Employee|MZ\\Employee\\][H:SyncModel]';
...
CREATE TABLE IF NOT EXISTS `Pedidos` (
  ...
  `FuncionarioID` INT NULL DEFAULT NULL COMMENT 'Funcionário que criou esse pedido[N:Funcionário][G:o][S:S]',
...
CREATE TABLE IF NOT EXISTS `Contas` (
  ...
  `Fonte` ENUM('Fixa', 'Variavel', 'Comissao', 'Salario') NOT NULL DEFAULT 'Fixa' COMMENT 'Fonte dos valores, comissão e salário só consegue pagar no vencimento ou após ele[G:a][N:Fonte dos valores]',
  ...
  `Estado` ENUM('Ativa', 'Paga', 'Cancelada', 'Desativada') NOT NULL DEFAULT 'N' COMMENT 'Informa o estado da conta[N:Estado][G:o]',
...
CREATE TABLE IF NOT EXISTS `Pagamentos` (
  ...
  `PagamentoID` INT NULL DEFAULT NULL COMMENT 'Informa o pagamento principal ou primeira parcela[G:o][N:Pagamento]',
  ...
  `FormaPagtoID` INT NULL DEFAULT NULL COMMENT 'Forma da pagamento do pedido ou conta[N:Forma de pagamento][G:a][S]',
  ...
  `DataAntecipacao` DATETIME NULL DEFAULT NULL COMMENT 'Data para o valor do pagamento ser antecipado[G:a][N:Data de antecipação]',
...
CREATE TABLE IF NOT EXISTS `Itens` (
  ...
  INDEX `FK_ProdPed_Pagamentos_PagamentoID_idx` (`PagamentoID` ASC),
...
CREATE TABLE IF NOT EXISTS `Auditoria` (
  ...
  `Autorizacao` VARCHAR(255) NULL DEFAULT NULL,
  `DataVisualizacao` DATETIME NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `Promocoes` (
  ...
  `BairroID` INT NULL DEFAULT NULL,
  `ZonaID` INT NULL DEFAULT NULL,
  ...
  `Agendamento` ENUM('Y', 'N') NOT NULL DEFAULT 'N' COMMENT 'Informa se essa promoção é um agendamento de preço, na data inicial o preço será aplicado, assim como a visibilidade do produto ou serviço será ativada[G:o][N:Agendamento]',
...
USE `GrandChef`$$
DROP TRIGGER IF EXISTS `Funcionarios_AFTER_INSERT` $$
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Funcionarios_AFTER_INSERT` AFTER INSERT ON `Funcionarios` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
	...
	SELECT ServidorID INTO LOCAL_ID FROM Sistema WHERE ID = '1';
	INSERT INTO `Registros` (`ServidorID`, `Tabela`, `Linha`, `Evento`, `Momento`) VALUES
		(COALESCE(@SERVER_ID, LOCAL_ID), 'Funcionarios', NEW.ID, 'Inserido', UNIX_TIMESTAMP());
	INSERT INTO `Mapeamentos` (`ServidorID`, `Tabela`, `De`, `Para`) VALUES
		(COALESCE(@SERVER_ID, LOCAL_ID), 'Funcionarios', COALESCE(@FROM_ID, NEW.ID), NEW.ID);
...
USE `GrandChef`$$
DROP TRIGGER IF EXISTS `Funcionarios_AFTER_UPDATE` $$
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Funcionarios_AFTER_UPDATE` AFTER UPDATE ON `Funcionarios` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
	...
	SELECT ServidorID INTO LOCAL_ID FROM Sistema WHERE ID = '1';
	INSERT INTO `Registros` (`ServidorID`, `Tabela`, `Linha`, `Evento`, `Momento`) VALUES
		(COALESCE(@SERVER_ID, LOCAL_ID), 'Funcionarios', NEW.ID, 'Atualizado', UNIX_TIMESTAMP());
...
USE `GrandChef`$$
DROP TRIGGER IF EXISTS `Funcionarios_AFTER_DELETE` $$
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Funcionarios_AFTER_DELETE` AFTER DELETE ON `Funcionarios` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
	...
	SELECT ServidorID INTO LOCAL_ID FROM Sistema WHERE ID = '1';
	INSERT INTO `Registros` (`ServidorID`, `Tabela`, `Linha`, `Evento`, `Momento`) VALUES
		(COALESCE(@SERVER_ID, LOCAL_ID), 'Funcionarios', OLD.ID, 'Deletado', UNIX_TIMESTAMP());
	DELETE FROM `Mapeamentos`
		WHERE `Tabela` = 'Funcionarios' AND `Para` = OLD.ID;
...
USE `GrandChef`$$
DROP TRIGGER IF EXISTS `Produtos_Pedidos_AFTER_INSERT` $$
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Produtos_Pedidos_AFTER_INSERT` AFTER INSERT ON `Produtos_Pedidos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
	...
	SELECT ServidorID INTO LOCAL_ID FROM Sistema WHERE ID = '1';
	INSERT INTO `Registros` (`ServidorID`, `Tabela`, `Linha`, `Evento`, `Momento`) VALUES
		(COALESCE(@SERVER_ID, LOCAL_ID), 'Produtos_Pedidos', NEW.ID, 'Inserido', UNIX_TIMESTAMP());
	INSERT INTO `Mapeamentos` (`ServidorID`, `Tabela`, `De`, `Para`) VALUES
		(COALESCE(@SERVER_ID, LOCAL_ID), 'Produtos_Pedidos', COALESCE(@FROM_ID, NEW.ID), NEW.ID);
...
USE `GrandChef`$$
DROP TRIGGER IF EXISTS `Produtos_Pedidos_AFTER_UPDATE` $$
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Produtos_Pedidos_AFTER_UPDATE` AFTER UPDATE ON `Produtos_Pedidos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
	...
	SELECT ServidorID INTO LOCAL_ID FROM Sistema WHERE ID = '1';
	INSERT INTO `Registros` (`ServidorID`, `Tabela`, `Linha`, `Evento`, `Momento`) VALUES
		(COALESCE(@SERVER_ID, LOCAL_ID), 'Produtos_Pedidos', NEW.ID, 'Atualizado', UNIX_TIMESTAMP());
...
USE `GrandChef`$$
DROP TRIGGER IF EXISTS `Produtos_Pedidos_AFTER_DELETE` $$
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Produtos_Pedidos_AFTER_DELETE` AFTER DELETE ON `Produtos_Pedidos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
	...
	SELECT ServidorID INTO LOCAL_ID FROM Sistema WHERE ID = '1';
	INSERT INTO `Registros` (`ServidorID`, `Tabela`, `Linha`, `Evento`, `Momento`) VALUES
		(COALESCE(@SERVER_ID, LOCAL_ID), 'Produtos_Pedidos', OLD.ID, 'Deletado', UNIX_TIMESTAMP());
	DELETE FROM `Mapeamentos`
		WHERE `Tabela` = 'Produtos_Pedidos' AND `Para` = OLD.ID;
