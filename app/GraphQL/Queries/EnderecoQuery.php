...
class EnderecoQuery extends Query
{
    ...
    {
        return Auth::check() && Auth::user()->can('endereco:view');
    }
    ...
    {
        return [
            'filter' => ['name' => 'filter', 'type' => GraphQL::type('EnderecoFilter')],
            'order' => ['name' => 'order', 'type' => GraphQL::type('EnderecoOrder')],
    ...
    public function resolve($root, $args, $context, ResolveInfo $resolveInfo, Closure $getSelectFields)
    {
        ...
        $query = Filter::apply(
            $args['filter'] ?? [],
            Endereco::with($fields->getRelations())->select($fields->getSelect())
        );
