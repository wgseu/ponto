...
class Juncao extends \MZ\Database\Helper
{
    ...
    public function update()
    {
        ...
        try {
            self::getDB()
                ...
                ->where('id', $this->getID())
                ->execute();
            $juncao = self::findByID($this->getID());
            $this->fromArray($juncao->toArray());
        } catch (\Exception $e) {
            throw $this->translate($e);
        }
     ...
     * @param  int $id id to find Junção
     * @return Juncao Self filled instance or empty when not found
     */
     ...
     * @param  int $id id to find Junção
     * @return Juncao A filled instance or empty when not found
     */
