...
class PacoteQuery extends Query
{
    ...
    {
        return [
            'filter' => ['name' => 'filter', 'type' => GraphQL::type('PacoteFilter')],
            'archived' => ['name' => 'archived', 'type' => Type::boolean()],
            'order' => ['name' => 'order', 'type' => GraphQL::type('PacoteOrder')],
    ...
    public function resolve($root, $args, $context, ResolveInfo $resolveInfo, Closure $getSelectFields)
    {
        ...
        $query = Pacote::with($fields->getRelations())->select($fields->getSelect());
        if (
            ($args['archived'] ?? false) &&
            auth()->check() &&
