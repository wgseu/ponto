...
class ZCategoria {
	...
	private static function validarCampos(&$categoria) {
		...
		if(strlen($categoria['categoriaid']) == 0)
		...
		else if(!is_numeric($categoria['categoriaid']))
			...
			else if(!is_null($_categoria->getCategoriaID()))
				$erros['categoriaid'] = 'A categoria informada já é uma subcategoria';
			else if($_categoria->getID() == $categoria['id'])
		...
		if(strlen($categoria['servico']) == 0)
			$categoria['servico'] = 'N';
		else if(!in_array($categoria['servico'], array('Y', 'N')))
