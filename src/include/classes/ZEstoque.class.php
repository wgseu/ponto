...
class ZEstoque {
	...
	private static function validarCampos(&$estoque) {
		...
		else if(!is_numeric($estoque['transacaoid']))
		...
		else if(!is_numeric($estoque['entradaid']))
		...
		else if(!is_numeric($estoque['fornecedorid']))
		...
		else if(!is_numeric($estoque['precocompra']))
		...
		if(strlen($estoque['cancelado']) == 0)
			$estoque['cancelado'] = 'N';
		else if(!in_array($estoque['cancelado'], array('Y', 'N')))
		...
		while (!$stack->isEmpty()) {
			...
			if($produto->getTipo() == ProdutoTipo::COMPOSICAO) {
				...
				foreach ($composicoes as $_composicao) {
					...
					else if($existe && $_composicao->getTipo() == ComposicaoTipo::ADICIONAL) {
						unset($ignore_composicoes[$_composicao->getID()]);
						$stack->push($_composicao);
					} else if ($_composicao->getTipo() != ComposicaoTipo::ADICIONAL)
						$stack->push($_composicao);
				}
