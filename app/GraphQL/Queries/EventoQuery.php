...
class EventoQuery extends Query
{
    ...
    {
        return Auth::check() && Auth::user()->can('evento:view');
    }

    public function type(): Type
    {
        return GraphQL::paginate('Evento');
    }

    public function args(): array
    {
        return [
            'filter' => ['name' => 'filter', 'type' => GraphQL::type('EventoFilter')],
            'order' => ['name' => 'order', 'type' => GraphQL::type('EventoOrder')],
    ...
    public function resolve($root, $args, $context, ResolveInfo $resolveInfo, Closure $getSelectFields)
    {
        ...
        $query = Filter::apply(
            $args['filter'] ?? [],
            Evento::with($fields->getRelations())->select($fields->getSelect())
        );
