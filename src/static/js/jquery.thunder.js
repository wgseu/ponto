(function ($) {
    ...
    $.thunder.settings = {
        version: '1.2.3',
        images: {
            loadingModal: '/static/images/loading_modal.gif',
            loadingGrid: '/static/images/loading_grid.gif',
            loadingForm: '/static/images/loading_form.gif'
        },
        confirm: {
            label: { yes: 'Yes', no: 'No' }
        },
    ...
    var methods = {
        message: {
            create: function ($this, cssClass, message, options) {
                ...
                if(cssClass.indexOf('error') < 0 && cssClass.indexOf('attention') < 0) {
                    $this.find('.thunder-error').remove();
                    $this.find('.thunder-attention').remove();
                } else if(cssClass.indexOf('error') >= 0 || cssClass.indexOf('attention') >= 0) {
                    $this.find('.thunder-notification').remove();
                }
                $last = $this.children('.thunder-notification').last();
                if($last.length == 0)
                ...
                $this.attr('class', 'thunder-notification');
                $this.hide();
                $this.empty();
