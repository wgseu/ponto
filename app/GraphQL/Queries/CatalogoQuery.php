...
class CatalogoQuery extends Query
{
    ...
    {
        return Auth::check() && Auth::user()->can('catalogo:view');
    }

    public function type(): Type
    {
        return GraphQL::paginate('Catalogo');
    }

    public function args(): array
    {
        return [
            'filter' => ['name' => 'filter', 'type' => GraphQL::type('CatalogoFilter')],
            'order' => ['name' => 'order', 'type' => GraphQL::type('CatalogoOrder')],
    ...
    public function resolve($root, $args, $context, ResolveInfo $resolveInfo, Closure $getSelectFields)
    {
        ...
        $query = Filter::apply(
            $args['filter'] ?? [],
            Catalogo::with($fields->getRelations())->select($fields->getSelect())
        );
