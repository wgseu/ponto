...
class ZPatrimonio {
	...
	private static function validarCampos(&$patrimonio) {
		...
		else if(!is_numeric($patrimonio['fornecedorid']))
		...
		else if($patrimonio['quantidade'] < 1)
		...
		else if(!in_array($patrimonio['estado'], array('Novo', 'Conservado', 'Ruim')))
		...
		if(strlen($patrimonio['ativo']) == 0)
			$patrimonio['ativo'] = 'N';
		else if(!in_array($patrimonio['ativo'], array('Y', 'N')))
