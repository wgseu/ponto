...
class SaldoQuery extends Query
{
    ...
    {
        return Auth::check() && Auth::user()->can('saldo:view');
    }

    public function type(): Type
    {
        return GraphQL::paginate('Saldo');
    }

    public function args(): array
    {
        return [
            'filter' => ['name' => 'filter', 'type' => GraphQL::type('SaldoFilter')],
            'order' => ['name' => 'order', 'type' => GraphQL::type('SaldoOrder')],
    ...
    public function resolve($root, $args, $context, ResolveInfo $resolveInfo, Closure $getSelectFields)
    {
        ...
        $query = Filter::apply(
            $args['filter'] ?? [],
            Saldo::with($fields->getRelations())->select($fields->getSelect())
        );
