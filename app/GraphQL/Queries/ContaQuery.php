...
class ContaQuery extends Query
{
    ...
    {
        return Auth::check() && Auth::user()->can('conta:view');
    }

    public function type(): Type
    {
        return GraphQL::paginate('Conta');
    }

    public function args(): array
    {
        return [
            'filter' => ['name' => 'filter', 'type' => GraphQL::type('ContaFilter')],
            'order' => ['name' => 'order', 'type' => GraphQL::type('ContaOrder')],
    ...
    public function resolve($root, $args, $context, ResolveInfo $resolveInfo, Closure $getSelectFields)
    {
        ...
        $query = Filter::apply(
            $args['filter'] ?? [],
            Conta::with($fields->getRelations())->select($fields->getSelect())
        );
