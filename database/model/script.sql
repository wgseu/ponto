...
CREATE TABLE IF NOT EXISTS `Carteiras` (
  ...
  `CaixaID` INT NULL DEFAULT NULL COMMENT 'Informa se esta carteira é um caixa da empresa de tipo local',
  ...
  `Transacao` DECIMAL(19,4) NOT NULL DEFAULT 0 COMMENT 'Valor cobrado pela operadora de pagamento para cada transação[N:Transação][G:a]',
  ...
  `Ambiente` ENUM('Teste', 'Producao') NULL DEFAULT NULL COMMENT 'Ambiente de execução da API usando o token[G:o][N:Ambiente]',
  ...
  INDEX `FK_Carteiras_Caixas_CaixaID_idx` (`CaixaID` ASC),
...
CREATE TABLE IF NOT EXISTS `Cartoes` (
  ...
  `Nome` VARCHAR(50) NOT NULL COMMENT 'Nome do cartão[N:Nome][G:a][S]',
  `Bandeira` VARCHAR(50) NOT NULL COMMENT 'Bandeira do cartão[G:a][N:Bandeira]',
  ...
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `UK_FormaPagto_Nome` (`FormaPagtoID` ASC, `Nome` ASC),
...
CREATE TABLE IF NOT EXISTS `Clientes` (
  ...
  `Senha` VARCHAR(40) NULL DEFAULT NULL COMMENT 'Senha embaralhada do cliente[N:Senha][G:a][P]',
...
CREATE TABLE IF NOT EXISTS `Prestadores` (
  ...
  `PrestadorID` INT NULL COMMENT 'Informa a empresa que gerencia os colaboradores, nulo para a empresa do próprio estabelecimento[G:o][N:Prestador]',
  `Vinculo` ENUM('Funcionario', 'Prestador', 'Autonomo') NOT NULL DEFAULT 'Funcionario' COMMENT 'Vínculo empregatício com a empresa, funcionário e autônomo são pessoas físicas, prestador é pessoa jurídica[G:o][N:Vínculo]',
  ...
  UNIQUE INDEX `FK_Prestadores_Clientes_ClienteID_uk` (`ClienteID` ASC),
...
CREATE TABLE IF NOT EXISTS `Zonas` (
  ...
  UNIQUE INDEX `UK_Bairro_Nome` USING BTREE (`BairroID`, `Nome`),
...
CREATE TABLE IF NOT EXISTS `Localizacoes` (
  ...
  `Tipo` ENUM('Casa', 'Apartamento') NOT NULL DEFAULT 'Casa' COMMENT 'Tipo de endereço Casa ou Apartamento[N:Tipo][G:o][F:self::TIPO_CASA]',
...
CREATE TABLE IF NOT EXISTS `Entregas` (
  ...
  `Latitude` DOUBLE NULL DEFAULT NULL COMMENT 'Ponto latitudinal para localização do entregador em tempo real[N:Latitude][G:a]',
  `Longitude` DOUBLE NULL DEFAULT NULL COMMENT 'Ponto longitudinal para localização do entregador em tempo real[N:Longitude][G:a]',
  `DataAtualizacao` DATETIME NULL DEFAULT NULL COMMENT 'Data de atualização da localização do entregador[G:a][N:Data de atualização]',
  `DataChegada` DATETIME NULL DEFAULT NULL COMMENT 'Data de chegada no estabelecimento[G:a][N:Data de chegada]',
  `DataSaida` DATETIME NOT NULL COMMENT 'Data e hora que o entregador saiu para entregar o pedido[N:Data de saida][G:a]',
  PRIMARY KEY (`ID`),
  INDEX `FK_Entregas_Prestadores_EntregadorID_idx` (`EntregadorID` ASC),
  CONSTRAINT `FK_Entregas_Prestadores_EntregadorID`
...
CREATE TABLE IF NOT EXISTS `Pedidos` (
  ...
  INDEX `FK_Pedidos_Entregas_EntregaID_idx` (`EntregaID` ASC),
  ...
  CONSTRAINT `FK_Pedidos_Prestadores_FechadorID`
    ...
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `FK_Pedidos_Entregas_EntregaID`
...
CREATE TABLE IF NOT EXISTS `Categorias` (
  ...
  `Ordem` INT NOT NULL DEFAULT 0 COMMENT 'Informa a ordem de exibição das categorias nas vendas[G:a][N:Ordem]',
...
CREATE TABLE IF NOT EXISTS `Produtos` (
  ...
  `Codigo` INT NOT NULL COMMENT 'Código do produto, deve ser único entre todos os produtos[N:Código][G:o]',
...
CREATE TABLE IF NOT EXISTS `Contas` (
  ...
  `Tipo` ENUM('Receita', 'Despesa') NOT NULL DEFAULT 'Despesa' COMMENT 'Tipo de conta se receita ou despesa[G:o][N:Tipo]',
  ...
  `Fonte` ENUM('Fixa', 'Variavel', 'Comissao', 'Remuneracao') NOT NULL DEFAULT 'Fixa' COMMENT 'Fonte dos valores, comissão e remuneração se pagar antes do vencimento, o valor será proporcional[G:a][N:Fonte dos valores]',
  ...
  `Modo` ENUM('Diario', 'Mensal') NOT NULL DEFAULT 'Mensal' COMMENT 'Modo de cobrança se diário ou mensal, a quantidade é definida em frequencia[G:o][N:Modo]',
  ...
  `Formula` ENUM('Simples', 'Composto') NOT NULL DEFAULT 'Composto' COMMENT 'Fórmula de juros que será cobrado em caso de atraso[G:o][N:Tipo de juros]',
  ...
  `Estado` ENUM('Ativa', 'Paga', 'Cancelada', 'Desativada') NOT NULL DEFAULT 'N' COMMENT 'Informa o estado da conta, desativa quando agrupa[N:Estado][G:o]',
  ...
  INDEX `IDX_Vencimento` (`Vencimento` ASC),
  INDEX `IDX_DataEmissao` (`DataEmissao` ASC),
...
CREATE TABLE IF NOT EXISTS `Cheques` (
  ...
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `UK_Folhas_Cheques_ChequeID_Numero` (`Numero` ASC),
...
CREATE TABLE IF NOT EXISTS `Pagamentos` (
  ...
  `Estado` ENUM('Aberto', 'Aguardando', 'Analise', 'Pago', 'Disputa', 'Devolvido', 'Cancelado') NOT NULL DEFAULT 'Y' COMMENT 'Informa qual o andamento do processo de pagamento[N:Estado][G:o][F:self::ESTADO_ABERTO]',
  ...
  INDEX `IDX_DataCompensacao` (`DataCompensacao` ASC),
  INDEX `IDX_DataLancamento` (`DataLancamento` ASC),
...
CREATE TABLE IF NOT EXISTS `Itens` (
  ...
  PRIMARY KEY (`ID`),
  INDEX `IDX_Itens_DataHora` (`DataHora` ASC),
...
CREATE TABLE IF NOT EXISTS `Grupos` (
  ...
  UNIQUE INDEX `UK_Grupos_Produto_Nome` (`ProdutoID` ASC, `Nome` ASC),
...
CREATE TABLE IF NOT EXISTS `Promocoes` (
  ...
  CONSTRAINT `FK_Promocoes_Categorias_CategoriaID`
    ...
    ON UPDATE RESTRICT)
ENGINE = InnoDB
COMMENT = 'Informa se há descontos nos produtos em determinados dias da semana, o preço pode subir ou descer e ser agendado para ser aplicado[N:Promoção|Promoções][G:a][L:CadastroProdutos][K:MZ\\Sale|MZ\\Sale\\][H:SyncModel]';
...
CREATE TABLE IF NOT EXISTS `Acessos` (
  ...
  CONSTRAINT `FK_Acessos_Permissoes_PermissaoID`
    ...
    ON UPDATE RESTRICT)
ENGINE = InnoDB
COMMENT = 'Permite acesso à uma determinada funcionalidade da lista de permissões[N:Acesso|Acessos][G:o][L:AlterarConfiguracoes][K:MZ\\Employee|MZ\\Employee\\][H:SyncModel]';
...
CREATE TABLE IF NOT EXISTS `Catalogos` (
  ...
  UNIQUE INDEX `UK_Fornecedor_CodigoBarras` (`FornecedorID` ASC),
...
CREATE TABLE IF NOT EXISTS `Formacoes` (
  ...
  UNIQUE INDEX `UK_Formacoes_ProdutoPedidoID_PacoteID` (`ItemID` ASC, `PacoteID` ASC),
...
CREATE TABLE IF NOT EXISTS `Mapeamentos` (
  ...
  UNIQUE INDEX `UK_Servidor_Tabela_De` (`ServidorID` ASC, `Tabela` ASC, `De` DESC),
...
CREATE TABLE IF NOT EXISTS `Registros` (
  ...
  `Evento` ENUM('Inserido', 'Atualizado', 'Deletado') NOT NULL COMMENT 'Tipo de evento que foi registrado[G:o][N:Evento]',
...
CREATE TABLE IF NOT EXISTS `Pontuacoes` (
  ...
  CONSTRAINT `FK_Pontuacoes_Clientes_ClienteID`
    ...
    ON UPDATE RESTRICT)
ENGINE = InnoDB
COMMENT = 'Informa os pontos ganhos e gastos por compras de produtos promocionais[N:Pontuação|Pontuações][G:a][L:CadastroProdutos][K:MZ\\Sale|MZ\\Sale\\][H:SyncModel]';
...
CREATE TABLE IF NOT EXISTS `Observacoes` (
  ...
  UNIQUE INDEX `UK_Produto_Descricao` (`ProdutoID` ASC, `Descricao` ASC),
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Setores_AFTER_INSERT` AFTER INSERT ON `Setores` FOR EACH ROW
BEGIN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Setores_AFTER_UPDATE` AFTER UPDATE ON `Setores` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Setores_AFTER_DELETE` AFTER DELETE ON `Setores` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Mesas_AFTER_INSERT` AFTER INSERT ON `Mesas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Mesas_AFTER_UPDATE` AFTER UPDATE ON `Mesas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Mesas_AFTER_DELETE` AFTER DELETE ON `Mesas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Sessoes_AFTER_INSERT` AFTER INSERT ON `Sessoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Sessoes_AFTER_UPDATE` AFTER UPDATE ON `Sessoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Sessoes_AFTER_DELETE` AFTER DELETE ON `Sessoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Caixas_AFTER_INSERT` AFTER INSERT ON `Caixas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Caixas_AFTER_UPDATE` AFTER UPDATE ON `Caixas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Caixas_AFTER_DELETE` AFTER DELETE ON `Caixas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Bancos_AFTER_INSERT` AFTER INSERT ON `Bancos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Bancos_AFTER_UPDATE` AFTER UPDATE ON `Bancos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Bancos_AFTER_DELETE` AFTER DELETE ON `Bancos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Carteiras_AFTER_INSERT` AFTER INSERT ON `Carteiras` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Carteiras_AFTER_UPDATE` AFTER UPDATE ON `Carteiras` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Carteiras_AFTER_DELETE` AFTER DELETE ON `Carteiras` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Integracoes_AFTER_INSERT` AFTER INSERT ON `Integracoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Integracoes_AFTER_UPDATE` AFTER UPDATE ON `Integracoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Integracoes_AFTER_DELETE` AFTER DELETE ON `Integracoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Formas_Pagto_AFTER_INSERT` AFTER INSERT ON `Formas_Pagto` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Formas_Pagto_AFTER_UPDATE` AFTER UPDATE ON `Formas_Pagto` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Formas_Pagto_AFTER_DELETE` AFTER DELETE ON `Formas_Pagto` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Cartoes_AFTER_INSERT` AFTER INSERT ON `Cartoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Cartoes_AFTER_UPDATE` AFTER UPDATE ON `Cartoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Cartoes_AFTER_DELETE` AFTER DELETE ON `Cartoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Funcoes_AFTER_INSERT` AFTER INSERT ON `Funcoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Funcoes_AFTER_UPDATE` AFTER UPDATE ON `Funcoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Funcoes_AFTER_DELETE` AFTER DELETE ON `Funcoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Clientes_AFTER_INSERT` AFTER INSERT ON `Clientes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Clientes_AFTER_UPDATE` AFTER UPDATE ON `Clientes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Clientes_AFTER_DELETE` AFTER DELETE ON `Clientes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Prestadores_AFTER_INSERT` AFTER INSERT ON `Prestadores` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Prestadores_AFTER_UPDATE` AFTER UPDATE ON `Prestadores` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Prestadores_AFTER_DELETE` AFTER DELETE ON `Prestadores` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Moedas_AFTER_INSERT` AFTER INSERT ON `Moedas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Moedas_AFTER_UPDATE` AFTER UPDATE ON `Moedas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Moedas_AFTER_DELETE` AFTER DELETE ON `Moedas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Paises_AFTER_INSERT` AFTER INSERT ON `Paises` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Paises_AFTER_UPDATE` AFTER UPDATE ON `Paises` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Paises_AFTER_DELETE` AFTER DELETE ON `Paises` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Estados_AFTER_INSERT` AFTER INSERT ON `Estados` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Estados_AFTER_UPDATE` AFTER UPDATE ON `Estados` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Estados_AFTER_DELETE` AFTER DELETE ON `Estados` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Cidades_AFTER_INSERT` AFTER INSERT ON `Cidades` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Cidades_AFTER_UPDATE` AFTER UPDATE ON `Cidades` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Cidades_AFTER_DELETE` AFTER DELETE ON `Cidades` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Bairros_AFTER_INSERT` AFTER INSERT ON `Bairros` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Bairros_AFTER_UPDATE` AFTER UPDATE ON `Bairros` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Bairros_AFTER_DELETE` AFTER DELETE ON `Bairros` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Zonas_AFTER_INSERT` AFTER INSERT ON `Zonas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Zonas_AFTER_UPDATE` AFTER UPDATE ON `Zonas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Zonas_AFTER_DELETE` AFTER DELETE ON `Zonas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Localizacoes_AFTER_INSERT` AFTER INSERT ON `Localizacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Localizacoes_AFTER_UPDATE` AFTER UPDATE ON `Localizacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Localizacoes_AFTER_DELETE` AFTER DELETE ON `Localizacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Comandas_AFTER_INSERT` AFTER INSERT ON `Comandas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Comandas_AFTER_UPDATE` AFTER UPDATE ON `Comandas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Comandas_AFTER_DELETE` AFTER DELETE ON `Comandas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Pedidos_AFTER_INSERT` AFTER INSERT ON `Pedidos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Pedidos_AFTER_UPDATE` AFTER UPDATE ON `Pedidos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Pedidos_AFTER_DELETE` AFTER DELETE ON `Pedidos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Categorias_AFTER_INSERT` AFTER INSERT ON `Categorias` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Categorias_AFTER_UPDATE` AFTER UPDATE ON `Categorias` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Categorias_AFTER_DELETE` AFTER DELETE ON `Categorias` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Unidades_AFTER_INSERT` AFTER INSERT ON `Unidades` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Unidades_AFTER_UPDATE` AFTER UPDATE ON `Unidades` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Unidades_AFTER_DELETE` AFTER DELETE ON `Unidades` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Origens_AFTER_INSERT` AFTER INSERT ON `Origens` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Origens_AFTER_UPDATE` AFTER UPDATE ON `Origens` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Origens_AFTER_DELETE` AFTER DELETE ON `Origens` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Operacoes_AFTER_INSERT` AFTER INSERT ON `Operacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Operacoes_AFTER_UPDATE` AFTER UPDATE ON `Operacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Operacoes_AFTER_DELETE` AFTER DELETE ON `Operacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Impostos_AFTER_INSERT` AFTER INSERT ON `Impostos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Impostos_AFTER_UPDATE` AFTER UPDATE ON `Impostos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Impostos_AFTER_DELETE` AFTER DELETE ON `Impostos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Tributacoes_AFTER_INSERT` AFTER INSERT ON `Tributacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Tributacoes_AFTER_UPDATE` AFTER UPDATE ON `Tributacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Tributacoes_AFTER_DELETE` AFTER DELETE ON `Tributacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Produtos_AFTER_INSERT` AFTER INSERT ON `Produtos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Produtos_AFTER_UPDATE` AFTER UPDATE ON `Produtos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Produtos_AFTER_DELETE` AFTER DELETE ON `Produtos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Servicos_AFTER_INSERT` AFTER INSERT ON `Servicos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Servicos_AFTER_UPDATE` AFTER UPDATE ON `Servicos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Servicos_AFTER_DELETE` AFTER DELETE ON `Servicos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Classificacoes_AFTER_INSERT` AFTER INSERT ON `Classificacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Classificacoes_AFTER_UPDATE` AFTER UPDATE ON `Classificacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Classificacoes_AFTER_DELETE` AFTER DELETE ON `Classificacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Contas_AFTER_INSERT` AFTER INSERT ON `Contas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Contas_AFTER_UPDATE` AFTER UPDATE ON `Contas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Contas_AFTER_DELETE` AFTER DELETE ON `Contas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Movimentacoes_AFTER_INSERT` AFTER INSERT ON `Movimentacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Movimentacoes_AFTER_UPDATE` AFTER UPDATE ON `Movimentacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Movimentacoes_AFTER_DELETE` AFTER DELETE ON `Movimentacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Creditos_AFTER_INSERT` AFTER INSERT ON `Creditos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Creditos_AFTER_UPDATE` AFTER UPDATE ON `Creditos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Creditos_AFTER_DELETE` AFTER DELETE ON `Creditos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
DROP TRIGGER IF EXISTS `Folhas_Cheques_AFTER_INSERT` $$
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Folhas_Cheques_AFTER_INSERT` AFTER INSERT ON `Folhas_Cheques` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
	...
	SELECT ServidorID INTO LOCAL_ID FROM Sistema WHERE ID = '1';
	INSERT INTO `Registros` (`ServidorID`, `Tabela`, `Linha`, `Evento`, `Momento`) VALUES
		(COALESCE(@SERVER_ID, LOCAL_ID), 'Folhas_Cheques', NEW.ID, 'Inserido', UNIX_TIMESTAMP());
	INSERT INTO `Mapeamentos` (`ServidorID`, `Tabela`, `De`, `Para`) VALUES
		(COALESCE(@SERVER_ID, LOCAL_ID), 'Folhas_Cheques', COALESCE(@FROM_ID, NEW.ID), NEW.ID);
...
USE `GrandChef`$$
DROP TRIGGER IF EXISTS `Folhas_Cheques_AFTER_UPDATE` $$
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Folhas_Cheques_AFTER_UPDATE` AFTER UPDATE ON `Folhas_Cheques` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
	...
	SELECT ServidorID INTO LOCAL_ID FROM Sistema WHERE ID = '1';
	INSERT INTO `Registros` (`ServidorID`, `Tabela`, `Linha`, `Evento`, `Momento`) VALUES
		(COALESCE(@SERVER_ID, LOCAL_ID), 'Folhas_Cheques', NEW.ID, 'Atualizado', UNIX_TIMESTAMP());
...
USE `GrandChef`$$
DROP TRIGGER IF EXISTS `Folhas_Cheques_AFTER_DELETE` $$
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Folhas_Cheques_AFTER_DELETE` AFTER DELETE ON `Folhas_Cheques` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
	...
	SELECT ServidorID INTO LOCAL_ID FROM Sistema WHERE ID = '1';
	INSERT INTO `Registros` (`ServidorID`, `Tabela`, `Linha`, `Evento`, `Momento`) VALUES
		(COALESCE(@SERVER_ID, LOCAL_ID), 'Folhas_Cheques', OLD.ID, 'Deletado', UNIX_TIMESTAMP());
	DELETE FROM `Mapeamentos`
		WHERE `Tabela` = 'Folhas_Cheques' AND `Para` = OLD.ID;
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Pagamentos_AFTER_INSERT` AFTER INSERT ON `Pagamentos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Pagamentos_AFTER_UPDATE` AFTER UPDATE ON `Pagamentos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Pagamentos_AFTER_DELETE` AFTER DELETE ON `Pagamentos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Itens_AFTER_INSERT` AFTER INSERT ON `Itens` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Itens_AFTER_UPDATE` AFTER UPDATE ON `Itens` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Itens_AFTER_DELETE` AFTER DELETE ON `Itens` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Funcionalidades_AFTER_INSERT` AFTER INSERT ON `Funcionalidades` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Funcionalidades_AFTER_UPDATE` AFTER UPDATE ON `Funcionalidades` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Funcionalidades_AFTER_DELETE` AFTER DELETE ON `Funcionalidades` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Modulos_AFTER_INSERT` AFTER INSERT ON `Modulos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Modulos_AFTER_UPDATE` AFTER UPDATE ON `Modulos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Modulos_AFTER_DELETE` AFTER DELETE ON `Modulos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Permissoes_AFTER_INSERT` AFTER INSERT ON `Permissoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Permissoes_AFTER_UPDATE` AFTER UPDATE ON `Permissoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Permissoes_AFTER_DELETE` AFTER DELETE ON `Permissoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Auditoria_AFTER_INSERT` AFTER INSERT ON `Auditoria` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Auditoria_AFTER_UPDATE` AFTER UPDATE ON `Auditoria` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Auditoria_AFTER_DELETE` AFTER DELETE ON `Auditoria` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Composicoes_AFTER_INSERT` AFTER INSERT ON `Composicoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Composicoes_AFTER_UPDATE` AFTER UPDATE ON `Composicoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Composicoes_AFTER_DELETE` AFTER DELETE ON `Composicoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Fornecedores_AFTER_INSERT` AFTER INSERT ON `Fornecedores` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Fornecedores_AFTER_UPDATE` AFTER UPDATE ON `Fornecedores` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Fornecedores_AFTER_DELETE` AFTER DELETE ON `Fornecedores` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Listas_AFTER_INSERT` AFTER INSERT ON `Listas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Listas_AFTER_UPDATE` AFTER UPDATE ON `Listas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Listas_AFTER_DELETE` AFTER DELETE ON `Listas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Compras_AFTER_INSERT` AFTER INSERT ON `Compras` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Compras_AFTER_UPDATE` AFTER UPDATE ON `Compras` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Compras_AFTER_DELETE` AFTER DELETE ON `Compras` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Requisitos_AFTER_INSERT` AFTER INSERT ON `Requisitos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Requisitos_AFTER_UPDATE` AFTER UPDATE ON `Requisitos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Requisitos_AFTER_DELETE` AFTER DELETE ON `Requisitos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Estoque_AFTER_INSERT` AFTER INSERT ON `Estoque` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Estoque_AFTER_UPDATE` AFTER UPDATE ON `Estoque` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Estoque_AFTER_DELETE` AFTER DELETE ON `Estoque` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Grupos_AFTER_INSERT` AFTER INSERT ON `Grupos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Grupos_AFTER_UPDATE` AFTER UPDATE ON `Grupos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Grupos_AFTER_DELETE` AFTER DELETE ON `Grupos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Propriedades_AFTER_INSERT` AFTER INSERT ON `Propriedades` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Propriedades_AFTER_UPDATE` AFTER UPDATE ON `Propriedades` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Propriedades_AFTER_DELETE` AFTER DELETE ON `Propriedades` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Pacotes_AFTER_INSERT` AFTER INSERT ON `Pacotes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Pacotes_AFTER_UPDATE` AFTER UPDATE ON `Pacotes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Pacotes_AFTER_DELETE` AFTER DELETE ON `Pacotes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Dispositivos_AFTER_INSERT` AFTER INSERT ON `Dispositivos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Dispositivos_AFTER_UPDATE` AFTER UPDATE ON `Dispositivos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Dispositivos_AFTER_DELETE` AFTER DELETE ON `Dispositivos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Impressoras_AFTER_INSERT` AFTER INSERT ON `Impressoras` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Impressoras_AFTER_UPDATE` AFTER UPDATE ON `Impressoras` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Impressoras_AFTER_DELETE` AFTER DELETE ON `Impressoras` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Promocoes_AFTER_INSERT` AFTER INSERT ON `Promocoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Promocoes_AFTER_UPDATE` AFTER UPDATE ON `Promocoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Promocoes_AFTER_DELETE` AFTER DELETE ON `Promocoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Acessos_AFTER_INSERT` AFTER INSERT ON `Acessos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Acessos_AFTER_UPDATE` AFTER UPDATE ON `Acessos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Acessos_AFTER_DELETE` AFTER DELETE ON `Acessos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Catalogos_AFTER_INSERT` AFTER INSERT ON `Catalogos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Catalogos_AFTER_UPDATE` AFTER UPDATE ON `Catalogos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Catalogos_AFTER_DELETE` AFTER DELETE ON `Catalogos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Informacoes_AFTER_INSERT` AFTER INSERT ON `Informacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Informacoes_AFTER_UPDATE` AFTER UPDATE ON `Informacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Informacoes_AFTER_DELETE` AFTER DELETE ON `Informacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Resumos_AFTER_INSERT` AFTER INSERT ON `Resumos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Resumos_AFTER_UPDATE` AFTER UPDATE ON `Resumos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Resumos_AFTER_DELETE` AFTER DELETE ON `Resumos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Formacoes_AFTER_INSERT` AFTER INSERT ON `Formacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Formacoes_AFTER_UPDATE` AFTER UPDATE ON `Formacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Formacoes_AFTER_DELETE` AFTER DELETE ON `Formacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Enderecos_AFTER_INSERT` AFTER INSERT ON `Enderecos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Enderecos_AFTER_UPDATE` AFTER UPDATE ON `Enderecos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Enderecos_AFTER_DELETE` AFTER DELETE ON `Enderecos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Horarios_AFTER_INSERT` AFTER INSERT ON `Horarios` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Horarios_AFTER_UPDATE` AFTER UPDATE ON `Horarios` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Horarios_AFTER_DELETE` AFTER DELETE ON `Horarios` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Valores_Nutricionais_AFTER_INSERT` AFTER INSERT ON `Valores_Nutricionais` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Valores_Nutricionais_AFTER_UPDATE` AFTER UPDATE ON `Valores_Nutricionais` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Valores_Nutricionais_AFTER_DELETE` AFTER DELETE ON `Valores_Nutricionais` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Transferencias_AFTER_INSERT` AFTER INSERT ON `Transferencias` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Transferencias_AFTER_UPDATE` AFTER UPDATE ON `Transferencias` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Transferencias_AFTER_DELETE` AFTER DELETE ON `Transferencias` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Patrimonios_AFTER_INSERT` AFTER INSERT ON `Patrimonios` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Patrimonios_AFTER_UPDATE` AFTER UPDATE ON `Patrimonios` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Patrimonios_AFTER_DELETE` AFTER DELETE ON `Patrimonios` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Juncoes_AFTER_INSERT` AFTER INSERT ON `Juncoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Juncoes_AFTER_UPDATE` AFTER UPDATE ON `Juncoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Juncoes_AFTER_DELETE` AFTER DELETE ON `Juncoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Regimes_AFTER_INSERT` AFTER INSERT ON `Regimes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Regimes_AFTER_UPDATE` AFTER UPDATE ON `Regimes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Regimes_AFTER_DELETE` AFTER DELETE ON `Regimes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Emitentes_AFTER_INSERT` AFTER INSERT ON `Emitentes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Emitentes_AFTER_UPDATE` AFTER UPDATE ON `Emitentes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Emitentes_AFTER_DELETE` AFTER DELETE ON `Emitentes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Notas_AFTER_INSERT` AFTER INSERT ON `Notas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Notas_AFTER_UPDATE` AFTER UPDATE ON `Notas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Notas_AFTER_DELETE` AFTER DELETE ON `Notas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Eventos_AFTER_INSERT` AFTER INSERT ON `Eventos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Eventos_AFTER_UPDATE` AFTER UPDATE ON `Eventos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Eventos_AFTER_DELETE` AFTER DELETE ON `Eventos` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Empresas_AFTER_INSERT` AFTER INSERT ON `Empresas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Empresas_AFTER_UPDATE` AFTER UPDATE ON `Empresas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Empresas_AFTER_DELETE` AFTER DELETE ON `Empresas` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Pontuacoes_AFTER_INSERT` AFTER INSERT ON `Pontuacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Pontuacoes_AFTER_UPDATE` AFTER UPDATE ON `Pontuacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Pontuacoes_AFTER_DELETE` AFTER DELETE ON `Pontuacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Telefones_AFTER_INSERT` AFTER INSERT ON `Telefones` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Telefones_AFTER_UPDATE` AFTER UPDATE ON `Telefones` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Telefones_AFTER_DELETE` AFTER DELETE ON `Telefones` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Observacoes_AFTER_INSERT` AFTER INSERT ON `Observacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Observacoes_AFTER_UPDATE` AFTER UPDATE ON `Observacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
...
USE `GrandChef`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GrandChef`.`Observacoes_AFTER_DELETE` AFTER DELETE ON `Observacoes` FOR EACH ROW
BEGIN
	DECLARE LOCAL_ID INT;
IF @@FOREIGN_KEY_CHECKS THEN
