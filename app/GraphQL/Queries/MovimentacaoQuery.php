...
class MovimentacaoQuery extends Query
{
    ...
    {
        return Auth::check() && Auth::user()->can('movimentacao:view');
    }

    public function type(): Type
    {
        return GraphQL::paginate('Movimentacao');
    }

    public function args(): array
    {
        return [
            'filter' => ['name' => 'filter', 'type' => GraphQL::type('MovimentacaoFilter')],
            'order' => ['name' => 'order', 'type' => GraphQL::type('MovimentacaoOrder')],
    ...
    public function resolve($root, $args, $context, ResolveInfo $resolveInfo, Closure $getSelectFields)
    {
        ...
        $query = Filter::apply(
            $args['filter'] ?? [],
            Movimentacao::with($fields->getRelations())->select($fields->getSelect())
        );
