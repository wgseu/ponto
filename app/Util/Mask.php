...
class Mask
{
    ...
    public static function money($value, $format = false, $currency = null)
    {
        ...
        /** @var Pais $pais */
        $pais = !is_null($empresa) ? $empresa->pais : null;
        $sep = !is_null($pais) ? $pais->entries->getEntry('currency', 'separator', '.') : '.';
        $dec = !is_null($pais) ? $pais->entries->getEntry('currency', 'decimal', ',') : ',';
        $number =  number_format($value, 2, $dec, $sep);
        if ($format) {
            if (is_null($currency)) {
                $currency = !is_null($pais) ? $pais->moeda : new Moeda(['formato' => 'R$ :value']);
            }
