...
--   Função ou cargo de um funcionário[N:Função|Funções][G:a][L:AlterarConfiguracoes][K:MZ\Employee|MZ\Employee\][H:SyncModel]
...
--   Funcionário que trabalha na empresa e possui uma determinada função[N:Funcionário|Funcionários][G:o][L:CadastroFuncionarios][K:MZ\Employee|MZ\Employee\][H:SyncModel]
...
--   Cheques lançados como pagamentos[N:Cheque|Cheques][G:o][L:Pagamento][K:MZ\Payment|MZ\Payment\][H:SyncModel]
...
--   Folhas de cheque de um pagamento[N:Folha de cheque|Folhas de cheques][G:a][L:Pagamento][K:MZ\Payment|MZ\Payment\][H:SyncModel]
...
--   Informa se há descontos nos produtos em determinados dias da semana, o preço pode subir ou descer[N:Promoção|Promoções][G:a][L:CadastroProdutos][K:MZ\Sale|MZ\Sale\][H:SyncModel]
...
--   Produtos, taxas e serviços do pedido, a alteração do estado permite o controle de produção[N:Item do pedido|Itens do pedido][G:o][L:Pagamento][K:MZ\Sale|MZ\Sale\][H:SyncModel]
  ...
  CONSTRAINT "FK_ProdPed_Servicos_ServicoID"
    ...
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT "FK_ProdPed_Pagamentos_PagamentoID"
...
CREATE INDEX "Produtos_Pedidos.FK_ProdPed_Pagamentos_PagamentoID_idx" ON "Produtos_Pedidos" ("PagamentoID");
CREATE TABLE "Contas"(
