...
/** ******  left menu  *********************** **/
function setActiveMenu(slide) {
    var url = window.location.pathname;
    var a = $('#sidebar-menu a[href="' + url + '"]');
    if(a.length == 0) {
        url = url.substring(0, url.lastIndexOf('/') + 1);
        a = $('#sidebar-menu a[href="' + url + '"]');
    }
    var li = a.parent('li');
    var ul = li.addClass('current-page').parent('ul');
    ul.parent().addClass('active');
    if(!$('body').hasClass('nav-md'))
        ul.parent().addClass('active-sm');
    if(slide) {
...
$(function () {
    ...
    $('#menu_toggle').click(function () {
        if ($('body').hasClass('nav-md')) {
            ...
            if ($('#sidebar-menu li').hasClass('selected')) {
