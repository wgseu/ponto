...
class DispositivoQuery extends Query
{
    ...
    {
        return Auth::check() && Auth::user()->can('dispositivo:view');
    }

    public function type(): Type
    {
        return GraphQL::paginate('Dispositivo');
    }
    ...
    public function resolve($root, $args, $context, ResolveInfo $resolveInfo, Closure $getSelectFields)
    {
        ...
        $query = Filter::apply(
            $args['filter'] ?? [],
            Dispositivo::with($fields->getRelations())->select($fields->getSelect())
        );
        $limit = $args['limit'] ?? 10;
        if (!isset($args['limit'])) {
