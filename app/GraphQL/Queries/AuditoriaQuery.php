...
class AuditoriaQuery extends Query
{
    ...
    {
        return Auth::check() && Auth::user()->can('auditoria:view');
    }

    public function type(): Type
    {
        return GraphQL::paginate('Auditoria');
    }

    public function args(): array
    {
        return [
            'filter' => ['name' => 'filter', 'type' => GraphQL::type('AuditoriaFilter')],
            'order' => ['name' => 'order', 'type' => GraphQL::type('AuditoriaOrder')],
    ...
    public function resolve($root, $args, $context, ResolveInfo $resolveInfo, Closure $getSelectFields)
    {
        ...
        $query = Filter::apply(
            $args['filter'] ?? [],
            Auditoria::with($fields->getRelations())->select($fields->getSelect())
        );
