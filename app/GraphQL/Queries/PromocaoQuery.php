...
class PromocaoQuery extends Query
{
    ...
    {
        return [
            'filter' => ['name' => 'filter', 'type' => GraphQL::type('PromocaoFilter')],
            'archived' => ['name' => 'archived', 'type' => Type::boolean()],
            'order' => ['name' => 'order', 'type' => GraphQL::type('PromocaoOrder')],
    ...
    public function resolve($root, $args, $context, ResolveInfo $resolveInfo, Closure $getSelectFields)
    {
        ...
        $query = Promocao::with($fields->getRelations())->select($fields->getSelect());
        if (
            ($args['archived'] ?? false) &&
            auth()->check() &&
