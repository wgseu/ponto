...
Upload.image.initialize = function(img_id) {
    ...
    $(img_id + ' .img-open').click(function(e){
        e.preventDefault();
        input = $(this).parent().parent().find('input[type=file]');
        input.trigger('click');
    });
...
Gerenciar.diversos.init = function(d1, doughnutData, meta_val, meta_max) {
    ...
    // end datepicker
    var myDoughnut = new Chart($("#canvas1")[0].getContext("2d"), {
        type: 'doughnut',
        data: doughnutData, 
        options: doptions
    });
...
Gerenciar.cliente.initField = function(input, field, tipo) {
    Gerenciar.common.autocomplete('/app/cliente/procurar', input, undefined,
        ...
        function (query) {
            return {busca: query, tipo: tipo, limite: 5, formatar: 1};
        }, 
        function (response) {
...
Gerenciar.cidade.fill = function(id, nome, estado, uf) {
    $('#cidade').val(id);
    $('#cidadeid').val(nome);
};
Gerenciar.bairro = {};
...
Gerenciar.localizacao.initForm = function(focus_ctrl) {
    ...
    $('#bairro').autocomplete({
        lookup: function (query, done) {
            $.get('/app/bairro/', {nome: query, estadoid: $('#estadoid').val(), cidade: $('#cidade').val()},
                function(response) {
                    var result = {suggestions: []};
                    if(response.status == 'ok') {
                        result = {
...
Gerenciar.produto.initFieldSelect = function(input, field, stock, selectFn) {
    Gerenciar.common.autocomplete('/app/produto/procurar', input, selectFn,
        ...
        function (query) {
            return {busca: query, estoque: stock};
        }, 
        function (response) {
...
var allLoaded = false;
    ...
    function procuraProdutos(query) {
        $('#product-list').empty();
        $('#product-list').append('<div class="loader">Carregando...</div>');
    ...
    function showProductInfo (item) {
        ...
        $('#product-info').on('hidden.bs.modal', function (ev) {
            $('#query').focus();
        });
    }
...
Gerenciar.produto.iFoodForm = function(focus_ctrl) {
...
Gerenciar.fornecedor.initField = function(input, field) {
    Gerenciar.common.autocomplete('/app/fornecedor/', input, undefined,
        ...
        function (query) {
            return {busca: query, limite: 5, format: 1};
        }, 
