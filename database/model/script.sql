...
-- Mon 17 Feb 2020 06:59:09 PM -03
...
CREATE TABLE IF NOT EXISTS `sessoes` (
  ...
  `cozinha_id` INT NOT NULL COMMENT 'Remo de cozinha que será trabalhado nessa sessão[G:a][N:Cozinha]',
  ...
  INDEX `FK_sessoes_cozinha_id_idx` (`cozinha_id` ASC),
  CONSTRAINT `FK_sessoes_cozinha_id`
...
CREATE TABLE IF NOT EXISTS `clientes` (
  ...
  `fornecedor` ENUM('Y', 'N') NOT NULL DEFAULT 'N' COMMENT 'Informe se o registro é de um fornecedor[G:o][N:Fornecedor][F:false]',
...
CREATE TABLE IF NOT EXISTS `integracoes` (
  ...
  `codigo` VARCHAR(45) NOT NULL COMMENT 'Nome do ícone do módulo integrador[G:o][N:Ícone][I:128x128|integracao|integracao.png]',
  ...
  `tipo` ENUM('pedido', 'login', 'dispositivo', 'pagamento', 'outros') NOT NULL COMMENT 'Tipo de integração[G:o][N:Tipo]',
...
CREATE TABLE IF NOT EXISTS `pedidos` (
  ...
  `data_pronto` DATETIME NULL DEFAULT NULL COMMENT 'Data e hora em que todos os itens do pedido foram marcados como pronto[G:a][N:Data que está pronto]',
  `data_entrega` DATETIME NULL DEFAULT NULL COMMENT 'Data e hora que o pedido foi entregue no endereço informado pelo cliente[N:Data de entrega][G:a]',
...
CREATE TABLE IF NOT EXISTS `pagamentos` (
  ...
  `total` DECIMAL(19,4) NOT NULL COMMENT 'Total recebido ou pago incluso as taxas[N:Total][G:o]',
  `taxas` DECIMAL(19,4) NOT NULL COMMENT 'Taxas cobradas pelas operadoras de cartões[N:Taxas][G:a]',
...
CREATE TABLE IF NOT EXISTS `itens` (
  ...
  `custo_aproximado` DECIMAL(19,4) NOT NULL DEFAULT 0 COMMENT 'Custo aproximado do produto[N:Custo aproximado][G:o][F:0]',
...
CREATE TABLE IF NOT EXISTS `compras` (
  ...
  `numero` VARCHAR(100) NULL DEFAULT NULL COMMENT 'Informa o número fiscal da compra[N:Número da compra][G:o]',
  ...
  `conta_id` INT NULL DEFAULT NULL COMMENT 'Conta que foi gerada para essa compra[G:a][N:Conta]',
  `documento_url` VARCHAR(200) NULL DEFAULT NULL COMMENT 'Informa o nome do documento no servidor do sistema[N:Documento][G:o][I:256x256|compra|compra.png]',
  ...
  UNIQUE INDEX `numero_UNIQUE` (`numero` ASC),
  INDEX `FK_compras_conta_id_idx` (`conta_id` ASC),
  ...
  CONSTRAINT `FK_compras_comprador_id`
    ...
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `FK_compras_conta_id`
    FOREIGN KEY (`conta_id`)
    REFERENCES `contas` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS `estoques` (
  ...
  `producao_id` INT NULL DEFAULT NULL COMMENT 'Informa o que foi produzido através dessa saida de estoque[G:a][N:Produção]',
  `produto_id` INT NOT NULL COMMENT 'Produto que entrou no estoque[N:Produto][G:o][S][S:S]',
  `compra_id` INT NULL DEFAULT NULL COMMENT 'Informa de qual compra originou essa entrada em estoque[G:o][N:Compra]',
  ...
  `custo_medio` DECIMAL(19,4) NULL DEFAULT NULL COMMENT 'Custo médio do produto quando essa movimentação de entrada foi realizada[G:o][N:Custo médio]',
  `estoque` DOUBLE NULL DEFAULT NULL COMMENT 'Quantidade em estoque desse produto até essa entrada[G:o][N:Quantidade em estoque]',
  ...
  INDEX `FK_estoques_producao_id_idx` (`producao_id` ASC),
  INDEX `FK_estoques_compra_id_idx` (`compra_id` ASC),
  ...
  CONSTRAINT `FK_estoques_setor_id`
    ...
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `FK_estoques_compra_id`
    FOREIGN KEY (`compra_id`)
    REFERENCES `compras` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `FK_estoques_producao_id`
...
CREATE TABLE IF NOT EXISTS `dispositivos` (
  ...
  `tipo` ENUM('computador', 'navegador', 'movel') NOT NULL DEFAULT 'computador' COMMENT 'Tipo de dispositivo[N:Tipo][G:o][S:S][F:self::TIPO_COMPUTADOR]',
  ...
  INDEX `FK_dispositivos_caixa_id_idx` (`caixa_id` ASC),
...
CREATE TABLE IF NOT EXISTS `horarios` (
  ...
  `cozinha_id` INT NULL DEFAULT NULL COMMENT 'Ramo ou tipo de cozinha do restaurante que funcionará nesse horário[G:a][N:Cozinha]',
  ...
  INDEX `FK_horarios_cozinha_id_idx` (`cozinha_id` ASC),
  ...
  CONSTRAINT `FK_horarios_funcao_id`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `FK_horarios_cozinha_id`
    FOREIGN KEY (`cozinha_id`)
    REFERENCES `cozinhas` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
...
CREATE TABLE IF NOT EXISTS `cupons` (
  ...
  `quantidade` INT NOT NULL COMMENT 'Quantidade de cupons fornecidos ou usados[G:a][N:Quantidade]',
  `disponivel` INT NOT NULL DEFAULT 0 COMMENT 'Quantidade de cupons disponíveis para uso[G:a][N:Quantidade disponível][F:0]',
  ...
  `cancelado` ENUM('Y', 'N') NOT NULL DEFAULT 'N' COMMENT 'Informa se o cupom foi cancelado[G:o][N:Cancelado][F:false]',
...
CREATE TABLE IF NOT EXISTS `avaliacoes` (
  ...
  `metrica_id` INT NULL DEFAULT NULL COMMENT 'Métrica de avaliação ou nulo para resumo da avaliação do pedido[G:a][N:Métrica]',
  `cliente_id` INT NOT NULL COMMENT 'Informa o cliente que avaliou esse pedido ou produto, obrigatório quando for avaliação de produto[G:o][N:Cliente]',
  `pedido_id` INT NOT NULL COMMENT 'Pedido que foi avaliado[G:o][N:Pedido]',
  ...
  `publico` ENUM('Y', 'N') NOT NULL DEFAULT 'N' COMMENT 'Informa se o comentário está público para visualização[G:o][N:Público][F:false]',
...
CREATE TABLE IF NOT EXISTS `cardapios` (
  ...
  `cozinha_id` INT NOT NULL COMMENT 'Permite mostrar o cardápio somente dessa cozinha[G:a][N:Cozinha]',
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `item_destino_UNIQUE` (`cozinha_id` ASC, `produto_id` ASC, `composicao_id` ASC, `pacote_id` ASC, `cliente_id` ASC, `integracao_id` ASC, `local` ASC),
  ...
  INDEX `FK_cardapios_produto_id_idx` (`produto_id` ASC),
  ...
  CONSTRAINT `FK_cardapios_cliente_id`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `FK_cardapios_cozinha_id`
    FOREIGN KEY (`cozinha_id`)
    REFERENCES `cozinhas` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
...
CREATE TABLE IF NOT EXISTS `saldos` (
  `id` INT NOT NULL AUTO_INCREMENT COMMENT 'Identificador do saldo[G:o]',
  `moeda_id` INT NOT NULL COMMENT 'Moeda do valor do saldo[G:a][N:Moeda]',
  `carteira_id` INT NOT NULL COMMENT 'Carteira que contém o saldo[G:a][N:Carteira]',
  `valor` DECIMAL(19,4) NOT NULL DEFAULT 0 COMMENT 'Valor do saldo total da carteira e moeda[G:o][N:Valor][F:0]',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `moeda_carteira_UNIQUE` (`moeda_id` ASC, `carteira_id` ASC),
  INDEX `FK_saldos_carteira_id_idx` (`carteira_id` ASC),
  CONSTRAINT `FK_saldos_moeda_id`
    FOREIGN KEY (`moeda_id`)
    REFERENCES `moedas` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `FK_saldos_carteira_id`
    FOREIGN KEY (`carteira_id`)
    REFERENCES `carteiras` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
ENGINE = InnoDB
COMMENT = 'Saldos dos pagamentos para cada carteira e moeda[N:Saldo|Saldos][G:o][K:App\\Models|Models\\][H:Model][L:null][ID:9]';
...
CREATE TABLE IF NOT EXISTS `conferencias` (
  `id` INT NOT NULL AUTO_INCREMENT COMMENT 'Identificador da conferência[G:o]',
  `funcionario_id` INT NOT NULL COMMENT 'Funcionário que está realizando a conferẽncia do estoque[G:o][N:Funcionário]',
  `numero` INT NOT NULL COMMENT 'Número da conferência, incrementado todo dia[G:o][N:Número da conferẽncia]',
  `produto_id` INT NOT NULL COMMENT 'Produto que está sendo conferido nesse setor[G:o][N:Produto]',
  `setor_id` INT NOT NULL COMMENT 'Setor em que o produto está localizado[G:o][N:Setor]',
  `quantidade` DOUBLE NOT NULL COMMENT 'Quantidade registrada do produto nesse setor[G:a][N:Quantidade registrada]',
  `conferido` DOUBLE NOT NULL COMMENT 'Quantidade conferida do produto nesse setor[G:a][N:Quantidade conferida]',
  `data_conferencia` DATETIME NOT NULL COMMENT 'Data em que a conferência foi realizada[G:a][N:Data de conferência]',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `produto_setor_numero_UNIQUE` (`produto_id` ASC, `setor_id` ASC, `numero` ASC),
  INDEX `FK_contagens_setor_id_idx` (`setor_id` ASC),
  INDEX `FK_conferencias_funcionario_id_idx` (`funcionario_id` ASC),
  CONSTRAINT `FK_contagens_produto_id0`
    FOREIGN KEY (`produto_id`)
    REFERENCES `produtos` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `FK_contagens_setor_id0`
    FOREIGN KEY (`setor_id`)
    REFERENCES `setores` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `FK_conferencias_funcionario_id`
    FOREIGN KEY (`funcionario_id`)
    REFERENCES `prestadores` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
ENGINE = InnoDB
COMMENT = 'Conferência diária de produto em cada setor[N:Conferência|Conferẽncias][G:a][K:App\\Models|Models\\][H:Model][L:null][ID:30]';
